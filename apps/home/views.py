...
def index(request):   
    ...
    # If a UserProfile object doesn't exist for the user, create a new one
    if user_profile is None:
        user_profile = models.UserProfile(user=user)
        user_profile.save()

    # Get the UserProfile object for the user
...
def setting(request):
    if request.method == 'POST':
        ...
        if site_control:
            ...
            request.user.profile.site_control = site_control
        ...
        if show_quick_draft:
            if show_quick_draft == 'on':
                request.user.profile.show_quick_draft = True
            else:
                request.user.profile.show_quick_draft = False
            request.user.profile.save()
...
def predict(request, ticker_value, number_of_days):
    ...
    try:
        # number_of_days = request.POST.get('days')   #if user choose number of days
    ...
    return render(request, "chart.html", context={ 'plot_div': plot_div, 
